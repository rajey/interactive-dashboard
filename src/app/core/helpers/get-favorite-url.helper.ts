export function getFavoriteUrl(favorite: { id: string; type: string }): string {
  if (!favorite && !favorite.type && !favorite.id) {
    return '';
  }

  let url = favorite.type + 's/' + favorite.id + '.json?fields=';
  ...
  switch (favorite.type) {
    ...
    default:
      url +=
        ...
        '!rowDimensions,!filterDimensions,!organisationUnitGroups,!itemOrganisationUnitGroups,!indicators,!dataElements,' +
        '!dataElementOperands,!dataElementGroups,!dataSets,!periods,!organisationUnitLevels,!organisationUnits';
  }
